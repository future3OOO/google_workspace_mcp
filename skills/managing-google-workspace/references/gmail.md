...
- **URL**: `{"url": "https://example.com/file.pdf"}` -- optionally add `"filename"` and `"mime_type"`. Remote URLs are fetched through the same safe download path and Gmail size limit as other Gmail attachment flows
