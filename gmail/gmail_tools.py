...
def _html_part_text_for_cid_scan(part: EmailMessage) -> str:
    """Return the decoded HTML body text for CID scanning, tolerating broken MIME.

    ``EmailMessage.get_content()`` can raise ``LookupError`` for unknown charsets
    and other errors for broken content-transfer-encoding. Preserved drafts may
    originate from external clients with malformed MIME, so we must not fail the
    whole extract pass over a single unparseable HTML part.
    """
    try:
        ...
        return ""
    except Exception:  # pragma: no cover - defensive fallback for exotic errors
        return ""
...
def _extract_preserved_attachments(
    ...
    for part in parsed_message.walk():
        if part.get_content_type() != "text/html":
            continue
        html_text = _html_part_text_for_cid_scan(part)
        for match in re.findall(r"cid:([^\"' >]+)", html_text):
            referenced_cids.add(f"<{match}>")

    def _is_attachment_like(
        ...
        if filename or disposition in {"attachment", "inline"}:
            return True
        # A bare Content-ID does not make a part an attachment; otherwise body
        # parts of multipart/related (which may carry a CID for structural
        # reasons) would be rebuilt as visible attachments on draft update. Only
        # treat CID-only parts as attachment-like when something in the message
        # actually references them via cid:... in HTML.
        return bool(content_id) and content_id in referenced_cids
    ...
    def _visit(part: EmailMessage, parent_type: Optional[str] = None) -> None:
        ...
        if part_type == "message/rfc822":
            ...
            if not _is_attachment_like(filename, disposition, content_id):
