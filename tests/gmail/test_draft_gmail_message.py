...
def test_extract_preserved_attachments_skips_cid_only_html_body_in_multipart_related():
    """HTML body parts in multipart/related often carry a Content-ID for structural
    reasons but are not attachments. They must not be reattached on draft update.
    """
    outer = EmailMessage(policy=SMTP)
    outer["Subject"] = "S"
    outer["To"] = "r@example.com"
    outer.set_content("plain fallback")
    outer.add_alternative(
        "<html><body><p>Hello world</p></body></html>",
        subtype="html",
    )
    # Assign a Content-ID to the HTML body part itself (no cid:... reference).
    html_body = outer.get_body(preferencelist=("html",))
    html_body["Content-ID"] = "<body-html>"
    # Add an inline image referenced by the HTML body — this one should be preserved.
    ...
    # Rewrite HTML so the image is referenced.
    for part in outer.walk():
        if part.get_content_type() == "text/html":
            part.set_content(
                '<html><body><p>Hello</p><img src="cid:logo"></body></html>',
                subtype="html",
            )
            part["Content-ID"] = "<body-html>"
            break

    reparsed = BytesParser(policy=policy.default).parsebytes(
        outer.as_bytes(policy=SMTP)
    )
    preserved = _extract_preserved_attachments(reparsed)

    filenames = [item["filename"] for item in preserved]
    cids = [item["content_id"] for item in preserved]
    assert "logo.png" in filenames
    # The HTML body with Content-ID <body-html> is NOT referenced via cid:...
    # in any HTML part, so it must not be rebuilt as an attachment.
    assert "<body-html>" not in cids


def test_extract_preserved_attachments_tolerates_unknown_html_charset():
    """An HTML part with an unknown charset must not crash CID scanning."""
    outer = EmailMessage(policy=SMTP)
    outer["Subject"] = "S"
    outer["To"] = "r@example.com"
    outer.set_content("plain fallback")
    ...
    # Replace the HTML part's Content-Type with an unknown charset so
    # ``EmailMessage.get_content()`` raises ``LookupError``.
    for part in outer.walk():
        if part.get_content_type() == "text/html":
            del part["Content-Type"]
            part["Content-Type"] = 'text/html; charset="not-a-real-charset"'
            break
    ...
    # Must not raise.
    preserved = _extract_preserved_attachments(reparsed)
    # No real attachments in this message.
    assert preserved == []
