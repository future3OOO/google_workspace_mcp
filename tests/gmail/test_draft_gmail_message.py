...
@pytest.mark.asyncio
async def test_update_gmail_draft_requires_recipient_when_thread_move_lookup_fails():
    mock_service = Mock()
    mock_service.users().drafts().update().execute.return_value = {"id": "draft123"}
    existing_message = EmailMessage(policy=SMTP)
    existing_message["Subject"] = "Old subject"
    existing_message["To"] = "old-recipient@example.com"
    existing_message.set_content("Old body")
    mock_service.users().drafts().get().execute.return_value = {
        "message": {
            "threadId": "old-thread",
    ...
    mock_service.users().threads().get().execute.side_effect = RuntimeError(
        "thread unavailable"
    )
    mock_service.users.return_value.drafts.return_value.update.reset_mock()

    with pytest.raises(UserInputError, match="Could not derive recipient"):
        await _unwrap(update_gmail_draft)(
            service=mock_service,
            user_google_email="user@example.com",
            draft_id="draft123",
...
def test_extract_preserved_attachments_keeps_message_rfc822_fallback_bytes():
    raw_payload = b"raw message/rfc822 payload"

    class MessageRfc822Part:
        def get_content_disposition(self):
            return None

        def get(self, _header):
            return None
        ...
        def get_content(self):
            return b"not parsed as an EmailMessage"

        def as_bytes(self, policy=None):
            assert policy is SMTP
            return raw_payload

    preserved = gmail_tools._extract_preserved_attachments(MessageRfc822Part())

    assert len(preserved) == 1
    assert preserved[0]["filename"] is None
    assert preserved[0]["mime_type"] == "message/rfc822"
    assert preserved[0]["_allow_missing_filename"] is True
    assert base64.b64decode(preserved[0]["content"]) == raw_payload


@pytest.mark.asyncio
async def test_update_gmail_draft_preserves_multiple_multipart_attachments_when_omitted():
    mock_service = Mock()
    mock_service.users().drafts().update().execute.return_value = {"id": "draft123"}
    existing_message = EmailMessage(policy=SMTP)
    existing_message["Subject"] = "Old subject"
    ...
    for filename in ("bundle-1.eml", "bundle-2.eml"):
        multipart_attachment = EmailMessage(policy=SMTP)
        multipart_attachment.set_content(f"Nested plain {filename}")
        multipart_attachment.add_alternative(
            f"<p>Nested html {filename}</p>", subtype="html"
        )
    ...
    assert "Draft updated with 2 attachment(s)! Draft ID: draft123" in result
    ...
    for attachment in attachments:
        assert attachment.get_content_type() == "multipart/alternative"
        assert attachment.is_multipart()
        assert attachment.get_content_disposition() == "attachment"
        assert [part.get_content_type() for part in attachment.iter_parts()] == [
            "text/plain",
            "text/html",
        ]
...
@pytest.mark.asyncio
async def test_update_gmail_draft_ignores_inline_body_part_without_attachment_identity():
    mock_service = Mock()
    mock_service.users().drafts().update().execute.return_value = {"id": "draft123"}
    existing_message = EmailMessage(policy=SMTP)
    existing_message["Subject"] = "Old subject"
    ...
    existing_message.add_alternative("<p>Old body</p>", subtype="html")
    existing_message.get_body(preferencelist=("html",))[
        "Content-Disposition"
    ] = "inline"
    mock_service.users().drafts().get().execute.return_value = {
        "message": {"raw": _encode_raw_message(existing_message)}
...
@pytest.mark.asyncio
async def test_update_gmail_draft_preserves_unmarked_mixed_leaf_when_omitted():
    mock_service = Mock()
    mock_service.users().drafts().update().execute.return_value = {"id": "draft123"}
    existing_message = EmailMessage(policy=SMTP)
    existing_message["Subject"] = "Old subject"
    ...
    calendar_part = EmailMessage(policy=SMTP)
    calendar_part.set_content(
        "BEGIN:VCALENDAR\r\nSUMMARY:Planning\r\nEND:VCALENDAR\r\n",
        subtype="calendar",
    )
    existing_message.attach(calendar_part)
    mock_service.users().drafts().get().execute.return_value = {
        "message": {"raw": _encode_raw_message(existing_message)}
    }
    ...
    assert [attachment.get_content_type() for attachment in attachments] == [
        "text/calendar"
    ]
